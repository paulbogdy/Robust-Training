...
class ContrastiveV6Trainer:
    def __init__(self, model_wrapper: ModelWrapper, alphabet, device, args):
        ...
        self.base_path = f'contrastive_v6_q{self.q}_a{str(self.alpha).replace(".", "_")}_t{str(self.temperature).replace(".", "_")}_p{projection_dim}'
    ...
    def train(self, train_loader, val_loader, save_path, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in pbar:
                ...
                cls_1 = outputs_perturbed_1.hidden_states[-1][:, 0, :]  # [CLS] embedding for unperturbed
                cls_2 = outputs_perturbed_2.hidden_states[-1][:, 0, :]      # [CLS] embedding for perturbed

                # Pass [CLS] embeddings through the projection head
                z_i = self.projection_head(cls_1)
                z_j = self.projection_head(cls_2)

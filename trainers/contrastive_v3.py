...
class ContrastiveV3Trainer:
    def __init__(self, model_wrapper: ModelWrapper, alphabet, device, args):
        ...
        self.base_path = f'contrastive_v3_q{self.q}_a{str(self.alpha).replace(".", "_")}_t{str(self.temperature).replace(".", "_")}_p{projection_dim}'
    ...
    def train(self, train_loader, val_loader, save_path, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in pbar:
                ...
                # Perturb inputs
                perturbed_inputs_1 = [perturb_sentence(sentence, self.alphabet, q=self.q) for sentence in unperturbed_inputs]
                perturbed_inputs_2 = [perturb_sentence(sentence, self.alphabet, q=self.q) for sentence in unperturbed_inputs]

                # Tokenize both perturbed and unperturbed inputs
                perturbed_tokenized_1 = self.model.tokenize(perturbed_inputs_1)
                perturbed_tokenized_2 = self.model.tokenize(perturbed_inputs_2)

                input_ids_perturbed_1 = perturbed_tokenized_1['input_ids'].to(self.device)
                attention_mask_perturbed_1 = perturbed_tokenized_1['attention_mask'].to(self.device)

                input_ids_perturbed_2 = perturbed_tokenized_2['input_ids'].to(self.device)
                attention_mask_perturbed_2 = perturbed_tokenized_2['attention_mask'].to(self.device)

                self.optimizer.zero_grad()
                outputs_perturbed_1 = self.model.forward(
                    input_ids=input_ids_perturbed_1,
                    attention_mask=attention_mask_perturbed_1,
                    output_hidden_states=True
                )
                perturbed_logits_1 = outputs_perturbed_1.logits

                outputs_perturbed_2 = self.model.forward(
                    input_ids=input_ids_perturbed_2,
                    attention_mask=attention_mask_perturbed_2,
                    output_hidden_states=True
                )
                perturbed_logits_2 = outputs_perturbed_2.logits
                
                # Calculate cross-entropy loss for unperturbed sample
                cross_entropy_loss = (self.loss_fn(perturbed_logits_1, labels) + self.loss_fn(perturbed_logits_2, labels)) / 2

                # Extract [CLS] embeddings from the last hidden state of both perturbed inputs
                cls_embedding_perturbed_1 = outputs_perturbed_1.hidden_states[-1][:, 0, :]  # [CLS] embedding for unperturbed
                cls_embedding_perturbed_2 = outputs_perturbed_2.hidden_states[-1][:, 0, :]      # [CLS] embedding for perturbed

                # Pass [CLS] embeddings through the projection head
                z_i = self.projection_head(cls_embedding_perturbed_1)
                z_j = self.projection_head(cls_embedding_perturbed_2)

...
class LabelContrastiveEmbeddingMBTrainer:
    def __init__(self, model_wrapper: ModelWrapper, device, args):
        ...
        self.epsilon = args.epsilon  # Epsilon parameter for contrastive loss
        ...
        self.base_path = f'label_contrastive_embedding_mb_q{self.q}_a{str(self.alpha).replace(".","_")}_e{str(self.epsilon).replace(".","_")}_t{str(self.temperature).replace(".", "_")}_p{projection_dim}_bank{self.bank_size}'
    ...
    def train(self, train_loader, val_loader, save_path, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in pbar:
                ...
                perturbation = self.epsilon * embeddings_matrix_grad
